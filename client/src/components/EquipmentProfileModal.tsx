...
export const EquipmentProfileModal: React.FC<EquipmentProfileModalProps> = ({
  ...
  // ─── حقول التعديل الشاملة (تشمل الكود وكل البيانات) ───────────────────
  ...
  // التعامل بالـ ID الرقمي للمشروع للربط مع السيرفر
  const [currentProjectId, setCurrentProjectId] = useState<string>(
    equipment.currentProjectId ? String(equipment.currentProjectId) : ''
  );
  
  // روابط الصور القابلة للتعديل بالكامل
  ...
  // قائمة المشاريع القادمة من الداتا بيز لتغذية القائمة المنسدلة
  ...
  // 🔄 جلب المشاريع لتغذية القائمة المنسدلة عند فتح المودال
  useEffect(() => {
    if (isOpen) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    // 📦 تجهيز كائن البيانات المحدثة بالكامل لإرساله للسيرفر
    const payload = {
        ...
        <div className="p-4 border-b border-solid border-slate-500/10 flex justify-between items-center sticky top-0 bg-inherit z-10">
          <div>
            ...
            <p className="text-xs text-slate-400 m-0 mt-1">
              الكود الحالي: <span className="font-black text-amber-500 uppercase tracking-wider">{equipment.code}</span>
            </p>
          </div>
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="aspect-square rounded-xl bg-slate-500/5 border border-slate-500/10 flex items-center justify-center overflow-hidden">
                    {equipment.frontImageUrl ? <img src={equipment.frontImageUrl} alt="أمامي" className="w-full h-full object-cover" /> : <span className="text-[10px] text-slate-500 font-bold">لا يوجد (أمامية)</span>}
                  </div>
                  <div className="aspect-square rounded-xl bg-slate-500/5 border border-slate-500/10 flex items-center justify-center overflow-hidden">
                    {equipment.backImageUrl ? <img src={equipment.backImageUrl} alt="خلفي" className="w-full h-full object-cover" /> : <span className="text-[10px] text-slate-500 font-bold">لا يوجد (خلفية)</span>}
                  </div>
                  <div className="aspect-square rounded-xl bg-slate-500/5 border border-slate-500/10 flex items-center justify-center overflow-hidden">
                    {equipment.codeImageUrl ? <img src={equipment.codeImageUrl} alt="كود" className="w-full h-full object-cover" /> : <span className="text-[10px] text-slate-500 font-bold">لا يوجد (كود/لوحة)</span>}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 bg-slate-500/5 p-4 rounded-xl border border-slate-500/10 text-sm">
                <div className="space-y-1">
                  <span className="text-xs text-slate-400 font-bold block">كود الآلية الموحد:</span>
                  <span className="font-black text-blue-600 dark:text-blue-400 uppercase">{equipment.code}</span>
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1 border-t border-slate-500/10 pt-2">
                  ...
                  <span className="font-black font-mono">{equipment.type === 'equipment' ? (equipment.serialNumber || '---') : (equipment.plateNumber || '---')}</span>
                </div>
                <div className="col-span-2 border-t border-slate-500/10 pt-2 space-y-1">
                  <span className="text-xs text-slate-400 font-bold block">الموقع الميداني الحالي:</span>
                ...
                <div className="col-span-2 border-t border-slate-500/10 pt-2 space-y-1">
                  ...
                  <span className={`inline-block mt-1 px-3 py-0.5 rounded-lg text-xs font-black text-white ${equipment.status === 'available' ? 'bg-emerald-600' : 'bg-red-600'}`}>
                    {equipment.status === 'available' ? 'جاهز للعمل الميداني' : 'معطلة حالياً'}
                  </span>
                </div>
              </div>
            </div>
          ) : (
            /* ──────────────── وضع التعديل الشامل (يفتح كأنه استمارة جديدة قابلة للتحرير) ──────────────── */
            <form onSubmit={handleUpdate} className="space-y-4 text-sm">
              
              {/* تعديل كود الآلية الفريد */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-400">كود الآلية الفريد (يمكن تعديله بحذر) <span className="text-red-500 font-black">*</span>:</label>
                <input 
                  type="text" 
                  required 
                  value={code} 
                  onChange={(e) => setCode(e.target.value)} 
                  className={`w-full px-3 py-2 rounded-xl border outline-none font-black tracking-wider uppercase focus:border-blue-500 ${isDarkMode ? 'bg-slate-950 border-slate-800 text-amber-400' : 'bg-slate-50 border-slate-200 text-blue-600'}`} 
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className={`w-full px-3 py-2 rounded-xl border outline-none focus:border-blue-500 font-bold ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`} 
                />
              </div>
              ...
              {/* 🏗️ القائمة المنسدلة الديناميكية للمشاريع - تظهر أسماء المشاريع حياً من الداتا بيز */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-400">الموقع أو المشروع الميداني التابع له:</label>
                <select 
                  value={currentProjectId} 
                  onChange={(e) => setCurrentProjectId(e.target.value)} 
                  className={`w-full px-3 py-2 rounded-xl border outline-none font-bold cursor-pointer focus:border-blue-500 ${isDarkMode ? 'bg-slate-950 border-slate-800 text-white' : 'bg-slate-50 border-slate-200 text-slate-800'}`}
                >
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsEditing(false)} className="text-xs font-bold text-slate-400 bg-transparent border-0 cursor-pointer">إلغاء الأمر</button>
