...
export const EquipmentProfileModal: React.FC<EquipmentProfileModalProps> = ({
  ...
  // تحديث الحقول الداخلية عندما تتغير المعدة أو المركبة الممررة
  useEffect(() => {
  ...
  // حفظ التعديلات الشاملة والنهائية
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm box-border w-full" dir="rtl">
      <div className="w-full max-w-xl rounded-xl border-4 border-solid border-black bg-white text-black shadow-[0_0_35px_rgba(0,0,0,0.6)] relative max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-4 border-b-4 border-solid border-black flex justify-between items-center sticky top-0 bg-white z-10">
          <div>
            <h3 className="text-xl font-black text-blue-900 m-0">
              {isEditing ? `تعديل بيانات ${equipment.type === 'equipment' ? 'المعدة' : 'المركبة'}` : `تفاصيل: ${equipment.name}`}
            </h3>
        ...
        <div className="p-4 md:p-6 space-y-6">
          ...
          {!isEditing ? (
            /* ──────────────── وضع المعاينة والعرض فقط ──────────────── */
            <div className="space-y-6">
              
              <div className="space-y-2">
                <span className="text-base font-black text-black block">ألبوم الصور (اضغط على المربع لإضافة أو استبدال الصورة فوراً):</span>
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="relative aspect-square rounded-lg bg-slate-100 border-2 border-solid border-black flex flex-col items-center justify-center overflow-hidden group">
                    {uploadingFront ? (
                      <span className="text-xs font-black text-blue-900 animate-pulse">جاري التحميل...</span>
                    ) : frontImageUrl ? (
                      <>
                        <img src={frontImageUrl} alt="أمامي" className="w-full h-full object-cover" />
                        {isAuthorized && (
                          <label className="absolute inset-0 bg-black/60 text-white text-[11px] font-black opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity">استبدال الصورة الأمامية</label>
                        )}
                      </>
                    ) : (
                      <>
                        <span className="text-xs text-black font-black text-center p-1">لا يوجد صورة أمامية</span>
                  ...
                  <div className="relative aspect-square rounded-lg bg-slate-100 border-2 border-solid border-black flex flex-col items-center justify-center overflow-hidden group">
                    {uploadingBack ? (
                      ...
                      <>
                        ...
                        {isAuthorized && (
                          <label className="absolute inset-0 bg-black/60 text-white text-[11px] font-black opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity">استبدال الصورة الخلفية</label>
                        )}
                      </>
                    ) : (
                      <>
                        <span className="text-xs text-black font-black text-center p-1">لا يوجد صورة خلفية</span>
                      </>
                    )}
                  ...
                  <div className="relative aspect-square rounded-lg bg-slate-100 border-2 border-solid border-black flex flex-col items-center justify-center overflow-hidden group">
                    {uploadingCode ? (
                      ...
                      <>
                        ...
                        {isAuthorized && (
                          <label className="absolute inset-0 bg-black/60 text-white text-[11px] font-black opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity">استبدال صورة الكود أو اللوحة</label>
                        )}
                      </>
                    ) : (
                      <>
                        <span className="text-xs text-black font-black text-center p-1">لا يوجد صورة كود أو لوحة</span>
                      </>
                    )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border-2 border-solid border-black text-base">
                <div className="space-y-1">
                  <span className="text-xs text-black font-black block">الكود:</span>
                  <span className="font-black text-blue-900 text-lg uppercase">{equipment.code}</span>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-black font-black block">الاسم:</span>
                ...
                <div className="space-y-1 border-t-2 border-solid border-black pt-2">
                  ...
                  <span className="font-black text-black">{equipment.model || 'غير مسجل'}</span>
                </div>
                <div className="space-y-1 border-t-2 border-solid border-black pt-2">
                  <span className="text-xs text-black font-black block">
                    {equipment.type === 'equipment' ? 'الرقم التسلسلي (S/N):' : 'رقم اللوحة:'}
                  </span>
                ...
                <div className="col-span-1 md:col-span-2 border-t-2 border-solid border-black pt-2 space-y-1">
                  <span className="text-xs text-black font-black block">الموقع الحالي:</span>
                  <span className="font-black text-blue-900 text-base">{equipment.projectName || 'الورشة المركزية (بدون مشروع حالي)'}</span>
                </div>
                <div className="col-span-1 md:col-span-2 border-t-2 border-solid border-black pt-2 space-y-1">
                  <span className="text-xs text-black font-black block">الحالة التشغيلية:</span>
                  <span className={`inline-block mt-1 px-4 py-1 rounded text-sm font-black text-white ${equipment.status === 'available' ? 'bg-emerald-700' : 'bg-red-700'}`}>
                    {equipment.status === 'available' ? 'جاهز للعمل' : 'معطلة وخارج الخدمة'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleUpdate} className="space-y-5 text-base">
              ...
              <div className="space-y-1.5">
                <label className="block text-base font-black text-black">الكود *:</label>
                <input 
                  type="text" 
                  required 
                  value={code} 
              ...
              <div className="space-y-1.5">
                <label className="block text-base font-black text-black">{equipment.type === 'equipment' ? 'اسم المعدة:' : 'اسم المركبة:'}</label>
                <input 
                  type="text" 
                  required 
                  value={name} 
              ...
              <div className="space-y-1.5">
                <label className="block text-base font-black text-black">الموقع أو المشروع الحالي:</label>
                <select 
              ...
              {/* قسم تعديل ورفع الصور */}
              <div className="p-4 rounded-lg border-2 border-dashed border-black bg-blue-50/50 space-y-4">
                <span className="text-base font-black text-blue-900 block">تحديث ألبوم الصور:</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  ...
                  <div className="flex flex-col items-center p-2 rounded border border-solid border-black bg-white">
                    <span className="text-[11px] font-black mb-1">صورة الكود أو اللوحة</span>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t-4 border-solid border-black">
                <button type="button" onClick={() => setIsEditing(false)} className="text-base font-black text-black bg-transparent border-0 cursor-pointer hover:underline px-3">إلغاء</button>
        ...
        {/* أزرار التحكم السفلية عند العرض */}
        {!isEditing && (
          <div className="p-4 border-t-4 border-solid border-black flex justify-end gap-3 bg-slate-100 sticky bottom-0">
            {isAuthorized && (
              ...
              >
                تعديل البيانات والصور والموقع
              </button>
            )}
            <button onClick={onClose} className="bg-black hover:bg-slate-900 text-white text-base font-black px-6 py-2.5 rounded-lg border-2 border-solid border-black cursor-pointer">إغلاق</button>
          </div>
        )}
