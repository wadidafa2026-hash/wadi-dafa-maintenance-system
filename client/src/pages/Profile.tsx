...
export const Profile: React.FC<ProfileProps> = ({ user, isDarkMode }) => {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    } catch (err) { setPasswordMsg({ text: 'خطأ في الاتصال، يرجى المحاولة لاحقاً', isError: true }); }
  };
  ...
  const handleCreateAdmin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchUsers();
      } else { setAdminMsg({ text: data.message || 'فشل إضافة الحساب', isError: true }); }
    } catch (err) { setAdminMsg({ text: 'حدث خطأ، يرجى التحقق من الشبكة', isError: true }); }
  };

  // حذف مشرف
  const handleDeleteUser = async (id: number) => {
    if (!window.confirm('هل أنت متأكد من سحب صلاحية هذا الحساب وحذفه نهائياً؟')) return;
    try {
      const res = await fetch(`${baseUrl}/api/auth/users/${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchUsers();
      } else { alert('فشل حذف الحساب'); }
    } catch (err) { console.error(err); }
  };
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setProjectMsg({ text: 'تمت إضافة المشروع بنجاح', isError: false });
        setNewProjectName('');
        fetchProjects();
      } else { setProjectMsg({ text: 'فشل إضافة المشروع', isError: true }); }
    } catch (err) { setProjectMsg({ text: 'حدث خطأ، يرجى التحقق من الشبكة', isError: true }); }
  };

  // حذف مشروع
  const handleDeleteProject = async (id: number) => {
    if (!window.confirm('هل أنت متأكد من حذف هذا المشروع؟ سيتم فك ارتباطه من المعدات والمركبات الحالية تلقائياً.')) return;
    try {
  ...
  return (
    <div className={`grid grid-cols-1 ${user.role === 'super_admin' ? 'lg:grid-cols-3' : 'max-w-2xl mx-auto'} gap-6 box-border w-full text-black`} dir="rtl">
      ...
      {/* العمود الأيمن: بطاقة المستخدم وتغيير كلمة المرور */}
      <div className="space-y-6">
          ...
          <div className="text-center space-y-4">
            ...
            <div className="pt-2">
              <span className={`inline-block px-4 py-1.5 rounded text-xs font-black border-2 border-solid ${user.role === 'super_admin' ? 'bg-red-100 text-red-900 border-red-700' : user.role === 'sub_admin' ? 'bg-blue-100 text-blue-900 border-blue-900' : 'bg-slate-100 text-slate-900 border-slate-700'}`}>
                {user.role === 'super_admin' ? 'مدير النظام العام' : user.role === 'sub_admin' ? 'مشرف صيانة فرعي' : 'صلاحية عرض فقط'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 rounded-xl border-4 border-solid border-black bg-white shadow-md">
          <h4 className="text-sm font-black text-blue-900 m-0 mb-4">تغيير كلمة المرور:</h4>
          <form onSubmit={handleChangePassword} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-blue-900 text-white text-sm font-black py-3 rounded-lg border-2 border-solid border-black cursor-pointer shadow">حفظ</button>
          </form>
        </div>
      </div>

      {/* العمود الأيسر: لوحة التحكم للمدير العام فقط */}
      {user.role === 'super_admin' && (
        <div className="lg:col-span-2 space-y-6">
          
          {/* لوحة تحكم وإدارة الحسابات */}
          <div className="p-6 rounded-xl border-4 border-solid border-black bg-white space-y-6 shadow-md">
            <div>
              <h3 className="text-base font-black text-blue-900 m-0">إدارة حسابات وصلاحيات المشرفين</h3>
            </div>
            ...
            <form onSubmit={handleCreateAdmin} className="p-4 rounded-xl bg-slate-50 border-2 border-solid border-black grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5"><label className="text-xs text-black block font-black">الاسم الكامل:</label><input type="text" required placeholder="مثال: المهندس أحمد" value={newAdminName} onChange={(e) => setNewAdminName(e.target.value)} className="w-full px-3 py-2.5 text-sm font-black rounded border-2 border-solid border-black" /></div>
              <div className="space-y-1.5"><label className="text-xs text-black block font-black">اسم المستخدم (باللغة الإنجليزية):</label><input type="text" required placeholder="مثال: ahmed_eng" value={newAdminUsername} onChange={(e) => setNewAdminUsername(e.target.value)} className="w-full px-3 py-2.5 text-sm font-black rounded border-2 border-solid border-black" /></div>
              <div className="space-y-1.5"><label className="text-xs text-black block font-black">كلمة المرور:</label><input type="password" required value={newAdminPassword} onChange={(e) => setNewAdminPassword(e.target.value)} className="w-full px-3 py-2.5 text-sm font-black rounded border-2 border-solid border-black" /></div>
              <div className="space-y-1.5">
                ...
                <select value={newAdminRole} onChange={(e) => setNewAdminRole(e.target.value as any)} className="w-full px-3 py-2.5 rounded border-2 border-solid border-black text-sm font-black bg-white">
                  ...
                  <option value="viewer">صلاحية عرض فقط</option>
                </select>
              </div>
              ...
              <div className="md:col-span-2 flex justify-end pt-2 border-t border-solid border-black/10">
                <button type="submit" className="bg-amber-500 text-white text-sm font-black px-6 py-2.5 rounded-lg border-2 border-solid border-black shadow-md">إضافة الحساب</button>
              </div>
            </form>

            <div className="space-y-3">
              <h4 className="text-sm font-black text-black m-0">الحسابات المسجلة:</h4>
                      ...
                      <tr key={u.id} className="hover:bg-slate-100 border-b border-solid border-black/20">
                        ...
                        <td className="p-3 text-center">
                          {u.username !== user.username ? (
                            <button onClick={() => handleDeleteUser(u.id)} className="bg-red-100 text-red-700 border border-solid border-red-700 rounded px-2 py-1 text-xs cursor-pointer hover:bg-red-200">حذف</button>
          ...
          {/* لوحة تحكم وإدارة المشاريع والمواقع */}
          <div className="p-6 rounded-xl border-4 border-solid border-black bg-white space-y-6 shadow-md">
            <div>
              <h3 className="text-base font-black text-blue-900 m-0">إدارة المشاريع والموقع الحالية</h3>
              <p className="text-xs text-black font-black m-0 mt-1">أي مشروع يتم إضافته هنا سيظهر تلقائياً في قائمة الخيارات عند تسجيل مركبة أو معدة جديدة.</p>
            </div>
            ...
            <form onSubmit={handleCreateProject} className="p-4 rounded-xl bg-slate-50 border-2 border-solid border-black flex flex-col md:flex-row gap-4 items-end">
              <div className="flex-1 space-y-1.5 w-full">
                <label className="text-xs text-black block font-black">اسم المشروع / الموقع الجديد:</label>
                <input type="text" required placeholder="مثال: المشروع الجديد - القطاع الجنوبي" value={newProjectName} onChange={(e) => setNewProjectName(e.target.value)} className="w-full px-3 py-2.5 text-sm font-black rounded border-2 border-solid border-black bg-white" />
              </div>
              {projectMsg.text && <p className="text-xs font-black p-2 rounded border border-solid bg-amber-50 md:hidden">{projectMsg.text}</p>}
              <button type="submit" className="bg-blue-900 hover:bg-blue-950 text-white text-sm font-black px-6 py-2.5 rounded-lg border-2 border-solid border-black shadow-md whitespace-nowrap w-full md:w-auto">إضافة المشروع</button>
            </form>
            ...
            <div className="space-y-3">
              <h4 className="text-sm font-black text-black m-0">المشاريع والمواقع المسجلة حالياً:</h4>
              <div className="border-2 border-solid border-black rounded-lg overflow-hidden text-sm bg-white">
                <table className="w-full text-right border-collapse">
                  <thead>
                    <tr className="bg-slate-900 text-white font-black">
                      <th className="p-3">اسم المشروع / الموقع</th>
                    ...
                    {projectsList.length > 0 ? (
                      projectsList.map((p) => (
                        <tr key={p.id} className="hover:bg-slate-100 border-b border-solid border-black/20">
                          ...
                          <td className="p-3 text-center">
                            <button onClick={() => handleDeleteProject(p.id)} className="bg-red-50 text-red-600 border border-solid border-red-300 rounded px-2 py-1 text-xs cursor-pointer hover:bg-red-100">حذف</button>
                          </td>
                        </tr>
                      ))
                    ) : (
                      <tr><td colSpan={2} className="text-center p-4 text-slate-500">لا توجد مشاريع مسجلة حالياً.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
